...
def pcnn(X, Y, T, scaling=True, batch_size=100, epochs=100, max_epochs=1, directory="tuner", fixed_model = False, noise_multiplier=1, seed = 1):
    ...
    # save models
    checkpoint_filepath_mx = f"{directory}/_{epsilon}_m_x"
    checkpoint_filepath_taux = f"{directory}/_{epsilon}_tau_x"

    mx_callbacks = [callback,
      tf.keras.callbacks.ModelCheckpoint(filepath=str(directory+checkpoint_filepath_mx), save_weights_only=False, monitor='val_loss', mode='min', save_freq="epoch", save_best_only=True, save_format='tf'),]
    tau_hat_callbacks = [callback,
      tf.keras.callbacks.ModelCheckpoint(filepath=str(directory+checkpoint_filepath_taux), save_weights_only=False, monitor='val_loss', mode='min', save_freq="epoch", save_best_only=True, save_format='tf'),]
    y_tilde_hat = []  # collect all the \tilde{Y}
    T_tilde_hat = []  # collect all the \tilde{T}
    m_x_hat = []  # collect all m_x_hat for print
    e_x_hat = []  # collect all e_x_hat for print
